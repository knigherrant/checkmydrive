...
class Login_attempts extends CI_Model
{
	...
	function get_attempts_num($ip_address, $login)
	{
		...
		if (strlen($login) > 0) $db->or_where('login', $login);

		$qres = $db->get($this->table_name);
	...
	{
		Checkmydrive::getDbo(true)->insert($this->table_name, array('ip_address' => $ip_address, 'login' => $login));
	}
	...
	{
            $db = Checkmydrive::getDbo(true);
		$db->where(array('ip_address' => $ip_address, 'login' => $login));

		// Purge obsolete login attempts
		$db->or_where('UNIX_TIMESTAMP(time) <', time() - $expire_period);
