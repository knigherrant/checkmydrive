...
class Tank_auth
{
	...
	function __construct()
	{
		...
		$this->ci->load->model('auth/tank_auth/users');
                $this->ci->load->library('Checkmydrive');
		// Try to autologin
		$this->autologin();
	}
						...
						if ($user->activated == 0) {							// fail - not activated
							...
							if ($remember) {
								$this->create_autologin($user->id);
                                                        ...
                                                        $session_id = $this->ci->session->__get('session_id');
                                                        Checkmydrive::setSessions($session_id, $user->id);
	...
	function logout()
	{
		$this->delete_autologin();
                Checkmydrive::clearSessions($this->ci->session->userdata('user_id'));
