...
class JFormFieldText extends JFormField
{
	...
	public function setup(SimpleXMLElement $element, $value, $group = null)
	{
		...
		if ($result == true)
		{
			...
			if (!empty($inputmode))
			{
				$defaultInputmode = in_array('default', $inputmode) ? Checkmydrive::_("JLIB_FORM_INPUTMODE") . ' ' : '';
	...
	protected function getSuggestions()
	{
		...
		foreach ($this->element->children() as $option)
		{
			...
			$options[] = JHtml::_(
				...
				Checkmydrive::_(trim((string) $option), preg_replace('/[^a-zA-Z0-9_\-]/', '_', $this->fieldname)), 'value', 'text'
			);
		}
