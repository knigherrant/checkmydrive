			...
			switch ($element->getName())
			{
				...
				case 'option':
					...
					$tmp = JHtml::_(
						...
						Checkmydrive::_(trim((string) $element), preg_replace('/[^a-zA-Z0-9_\-]/', '_', $this->fieldname)), 'value', 'text',
						$disabled
					);
				...
				case 'group':
					...
					{
						$label = Checkmydrive::_($groupLabel);
					}
					...
					foreach ($element->children() as $option)
					{
						...
						// Create a new option object based on the <option /> element.
						$tmp = JHtml::_(
							'select.option', ($option['value']) ? (string) $option['value'] : Checkmydrive::_(trim((string) $option)),
							Checkmydrive::_(trim((string) $option)), 'value', 'text', $disabled
						);
