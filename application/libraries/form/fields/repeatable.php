...
class JFormFieldRepeatable extends JFormField
{
	...
	protected function getInput()
	{
		...
		foreach ($subForm->getFieldset($attributes->name . '_modal') as $field)
		{
			...
			$str[] = '<br /><small style="font-weight:normal">' . Checkmydrive::_($field->description) . '</small>';
			$str[] = '</th>';
		}
		...
		$select = (string) $this->element['select'] ? Checkmydrive::_((string) $this->element['select']) : Checkmydrive::_('JLIB_FORM_BUTTON_SELECT');
		...
		Checkmydrive::script('JAPPLY');
		Checkmydrive::script('JCANCEL');
		return implode("\n", $str);
	}
}
