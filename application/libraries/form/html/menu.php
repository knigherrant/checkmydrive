...
abstract class JHtmlMenu
{
	...
	public static function menuitems($config = array())
	{
		if (empty(static::$items))
		{
			...
			foreach ($menus as &$menu)
			{
				...
				static::$items[] = JHtml::_('select.option', $menu->value . '.1', Checkmydrive::_('JLIB_HTML_ADD_TO_THIS_MENU'));
	...
	public static function ordering(&$row, $id)
	{
		...
		{
			$ordering = '<input type="hidden" name="ordering" value="' . $row->ordering . '" />' . Checkmydrive::_('JGLOBAL_NEWITEMSLAST_DESC');
		}
	...
	public static function linkoptions($all = false, $unassigned = false)
	{
		...
		if ($all | $unassigned)
		{
			$mitems[] = JHtml::_('select.option', '<OPTGROUP>', Checkmydrive::_('JOPTION_MENUS'));

			if ($all)
			{
				$mitems[] = JHtml::_('select.option', 0, Checkmydrive::_('JALL'));
			}

			if ($unassigned)
			{
				$mitems[] = JHtml::_('select.option', -1, Checkmydrive::_('JOPTION_UNASSIGNED'));
			}
