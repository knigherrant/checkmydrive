...
abstract class JHtml
{
						...
						else
						{
							...
							if (strpos($file, '/'))
							{
								...
								if (strpos($file, '/'))
								{
									...
									{
										$md5 = dirname($path) . '/MD5SUM';
										$includes[] = Checkmydrive::root(true) . "/media/$extension/$element/$folder/$file" .
											(file_exists($md5) ? ('?' . file_get_contents($md5)) : '');
									...
									{
										$md5 = dirname($path) . '/MD5SUM';
										$includes[] = Checkmydrive::root(true) . "/media/$extension/$folder/$element/$file" .
											(file_exists($md5) ? ('?' . file_get_contents($md5)) : '');
									...
									{
										$md5 = dirname($path) . '/MD5SUM';
										$includes[] = Checkmydrive::root(true) . "/templates/$template/$folder/system/$element/$file" .
											(file_exists($md5) ? ('?' . file_get_contents($md5)) : '');
									...
									{
										$md5 = dirname($path) . '/MD5SUM';
										$includes[] = Checkmydrive::root(true) . "/media/system/$folder/$element/$file" .
											(file_exists($md5) ? ('?' . file_get_contents($md5)) : '');
								...
								else
								{
									...
									{
										$md5 = dirname($path) . '/MD5SUM';
										$includes[] = Checkmydrive::root(true) . "/media/$extension/$folder/$file" .
											(file_exists($md5) ? ('?' . file_get_contents($md5)) : '');
									...
									{
										$md5 = dirname($path) . '/MD5SUM';
										$includes[] = Checkmydrive::root(true) . "/templates/$template/$folder/system/$file" .
											(file_exists($md5) ? ('?' . file_get_contents($md5)) : '');
									...
									{
										$md5 = dirname($path) . '/MD5SUM';
										$includes[] = Checkmydrive::root(true) . "/media/system/$folder/$file" .
											(file_exists($md5) ? ('?' . file_get_contents($md5)) : '');
			...
			else
			{
				foreach ($potential as $strip)
				{
					...
					foreach ($files as $file)
					{
						...
						{
							$md5 = dirname($path) . '/MD5SUM';
							$includes[] = Checkmydrive::root(true) . "/$file" .
								(file_exists($md5) ? ('?' . file_get_contents($md5)) : '');
	 ...
	 * @param   int     $translate  If true will pass texts through Checkmydrive.
	...
	public static function tooltipText($title = '', $content = '', $translate = 1, $escape = 1)
	{
		...
		// Pass texts through the Checkmydrive.
		if ($translate)
		{
			$title = Checkmydrive::_($title);
			$content = Checkmydrive::_($content);
		}
