...
abstract class JHtmlGrid
{
	...
	public static function boolean($i, $value, $taskOn = null, $taskOff = null)
	{
		...
		// Build the title.
		$title = ($value) ? Checkmydrive::_('JYES') : Checkmydrive::_('JNO');
		$title = JHtml::tooltipText($title, Checkmydrive::_('JGLOBAL_CLICK_TO_TOGGLE_STATE'), 0);
	...
	public static function id($rowNum, $recId, $checkedOut = false, $name = 'cid')
	{
		return $checkedOut ? '' : '<input type="checkbox" id="cb' . $rowNum . '" name="' . $name . '[]" value="' . $recId
			. '" onclick="jSont.isChecked(this.checked);" />';
	}
	...
	public static function published($value, $i, $img1 = 'tick.png', $img0 = 'publish_x.png', $prefix = '')
	{
		...
		$alt = $value ? Checkmydrive::_('JPUBLISHED') : Checkmydrive::_('JUNPUBLISHED');
		$action = $value ? Checkmydrive::_('JLIB_HTML_UNPUBLISH_ITEM') : Checkmydrive::_('JLIB_HTML_PUBLISH_ITEM');

		return '<a href="#" onclick="return jSont.listItemTask(\'cb' . $i . '\',\'' . $prefix . $task . '\')" title="' . $action . '">'
	 ...
	 * @param   string  $published     The Checkmydrive string for published
	 * @param   string  $unpublished   The Checkmydrive string for Unpublished
	 * @param   string  $archived      The Checkmydrive string for Archived
	 * @param   string  $trashed       The Checkmydrive string for Trashed
	 *
	 * @return  string
	 *
	 * @since   1.5
	...
	{
		$state = array('' => '- ' . Checkmydrive::_('JLIB_HTML_SELECT_STATE') . ' -', 'P' => Checkmydrive::_($published), 'U' => Checkmydrive::_($unpublished));

		if ($archived)
		{
			$state['A'] = Checkmydrive::_($archived);
		}

		if ($trashed)
		{
			$state['T'] = Checkmydrive::_($trashed);
		}
	...
	public static function order($rows, $image = 'filesave.png', $task = 'saveorder')
	{
		return '<a href="javascript:saveorder(' . (count($rows) - 1) . ', \'' . $task . '\')" rel="tooltip" class="saveorder btn btn-micro pull-right" title="'
			. Checkmydrive::_('JLIB_HTML_SAVE_ORDER') . '"><i class="icon-menu-2"></i></a>';
	}
	...
	protected static function _checkedOut(&$row, $overlib = true)
	{
		...
		if ($overlib)
		{
			...
			$date = JHtml::_('date', $row->checked_out_time, Checkmydrive::_('DATE_FORMAT_LC1'));
