...
abstract class JHtmlAccess
{
	...
	public static function level($name, $selected, $attribs = '', $params = true, $id = false)
	{
		...
		elseif ($params)
		{
			array_unshift($options, JHtml::_('select.option', '', Checkmydrive::_('JOPTION_ACCESS_SHOW_ALL_LEVELS')));
		}
	...
	public static function usergroup($name, $selected, $attribs = '', $allowAll = true)
	{
		...
		if ($allowAll)
		{
			array_unshift($options, JHtml::_('select.option', '', Checkmydrive::_('JOPTION_ACCESS_SHOW_ALL_GROUPS')));
		}
	...
	public static function actions($name, $selected, $component, $section = 'global')
	{
		...
		for ($i = 0, $n = count($actions); $i < $n; $i++)
		{
			...
			$html[] = '			' . $checked . ' />';
			$html[] = '		<label for="' . $eid . '">';
			$html[] = '			' . Checkmydrive::_($item->title);
			$html[] = '		</label>';
			$html[] = '	</li>';
		}
