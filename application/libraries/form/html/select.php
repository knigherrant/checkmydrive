...
abstract class JHtmlSelect
{
	...
	{
		$arr = array(JHtml::_('select.option', '0', Checkmydrive::_($no)), JHtml::_('select.option', '1', Checkmydrive::_($yes)));
	 ...
	 *                                of selected options. Default: none.
	 *                               -list.translate: Boolean. If set, text and labels are translated via
	 *                                Checkmydrive::_(). Default is false.
	...
	public static function options($arr, $optKey = 'value', $optText = 'text', $selected = null, $translate = false)
	{
		...
		foreach ($arr as $elementKey => &$element)
		{
			...
			{
				$html .= $baseIndent . '<optgroup label="' . ($options['list.translate'] ? Checkmydrive::_($text) : $text) . '">' . $options['format.eol'];
				$baseIndent = str_repeat($options['format.indent'], ++$options['format.depth']);
			}
		...
		$translate = false)
	{
		...
		{
			$k = $obj->$optKey;
			$t = $translate ? Checkmydrive::_($obj->$optText) : $obj->$optText;
