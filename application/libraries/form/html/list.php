...
abstract class JHtmlList
{
	...
	public static function images($name, $active = null, $javascript = null, $directory = null, $extensions = "bmp|gif|jpg|png")
	{
		...
		$imageFiles = new DirectoryIterator(BASEPATH . '/' . $directory);
		$images = array(JHtml::_('select.option', '', Checkmydrive::_('JOPTION_SELECT_IMAGE')));
	...
	public static function genericordering($query, $chop = 30)
	{
		...
		if (empty($items))
		{
			$options[] = JHtml::_('select.option', 1, Checkmydrive::_('JOPTION_ORDER_FIRST'));

			return $options;
		}

		$options[] = JHtml::_('select.option', 0, '0 ' . Checkmydrive::_('JOPTION_ORDER_FIRST'));

		for ($i = 0, $n = count($items); $i < $n; $i++)
		{
			$items[$i]->text = Checkmydrive::_($items[$i]->text);
		...
		$options[] = JHtml::_('select.option', $items[$i - 1]->value + 1, ($items[$i - 1]->value + 1) . ' ' . Checkmydrive::_('JOPTION_ORDER_LAST'));
	...
	public static function ordering($name, $query, $attribs = null, $selected = null, $neworder = null)
	{
		...
		else
		{
			if ($neworder > 0)
			{
				$text = Checkmydrive::_('JGLOBAL_NEWITEMSLAST_DESC');
			}
			elseif ($neworder <= 0)
			{
				$text = Checkmydrive::_('JGLOBAL_NEWITEMSFIRST_DESC');
			}
		...
		$id = false)
	{
		...
		if ($none)
		{
			$pos[''] = Checkmydrive::_('JNONE');
		}

		if ($center)
		{
			$pos['center'] = Checkmydrive::_('JGLOBAL_CENTER');
		}

		if ($left)
		{
			$pos['left'] = Checkmydrive::_('JGLOBAL_LEFT');
		}

		if ($right)
		{
			$pos['right'] = Checkmydrive::_('JGLOBAL_RIGHT');
		}
