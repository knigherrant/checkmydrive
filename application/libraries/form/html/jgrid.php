...
abstract class JHtmlJGrid
{
		...
		$inactive_class = '', $enabled = true, $translate = true, $checkbox = 'cb')
	{
		...
		if ($tip)
		{
			...
			$title = $translate ? Checkmydrive::_($title) : $title;
			$title = Checkmydrive::_($title);
		}

		if ($enabled)
		{
			$html[] = '<a class="btn btn-micro' . ($active_class == 'publish' ? ' active' : '') . ($tip ? ' hasTooltip' : '') . '"';
			$html[] = ' href="javascript:void(0);" onclick="return jSont.listItemTask(\'' . $checkbox . $i . '\',\'' . $prefix . $task . '\')"';
	...
	public static function published($value, $i, $prefix = '', $enabled = true, $checkbox = 'cb', $publish_up = null, $publish_down = null)
	{
		...
		if ($publish_up || $publish_down)
		{
			...
			if ($publish_up)
			{
				$tips[] = Checkmydrive::sprintf('JLIB_HTML_PUBLISHED_START', $publish_up->format(JDate::$format, true));
			}

			if ($publish_down)
			{
				$tips[] = Checkmydrive::sprintf('JLIB_HTML_PUBLISHED_FINISHED', $publish_down->format(JDate::$format, true));
			}
			...
			foreach ($states as $key => $state)
			{
				...
				if ($tip)
				{
					$states[$key][1] = Checkmydrive::_($states[$key][1]);
					$states[$key][2] = Checkmydrive::_($states[$key][2]) . '<br />' . $tip;
					$states[$key][3] = Checkmydrive::_($states[$key][3]) . '<br />' . $tip;
					$states[$key][4] = true;
				}
			}
	...
	public static function checkedout($i, $editorName, $time, $prefix = '', $enabled = false, $checkbox = 'cb')
	{
		...
		$text = $editorName . '<br />' . JHtml::_('date', $time, Checkmydrive::_('DATE_FORMAT_LC')) . '<br />' . JHtml::_('date', $time, 'H:i');
		$active_title = JHtml::tooltipText(Checkmydrive::_('JLIB_HTML_CHECKIN'), $text, 0);
		$inactive_title = JHtml::tooltipText(Checkmydrive::_('JLIB_HTML_CHECKED_OUT'), $text, 0);

		return static::action(
			$i, 'checkin', $prefix, Checkmydrive::_('JLIB_HTML_CHECKED_OUT'), $active_title, $inactive_title, true, 'checkedout',
