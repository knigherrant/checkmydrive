...
class JForm
{
	...
	public function bind($data)
	{
		...
		foreach ($data as $k => $v)
		{
			...
			elseif (is_object($v) || JFormHelper::isAssociative($v))
			{
	...
	public function loadFile($file, $reset = true, $xpath = false)
	{
		...
		if (!is_file($file))
		{
			...
			$file = JFolder::find(self::addFormPath(), strtolower($file) . '.xml');
		...
		if ($value === null)
		{
			...
			if (($translate = $element['translate_default']) && ((string) $translate == 'true' || (string) $translate == '1'))
			{
				...
				if ($lang->hasKey($default))
				{
					...
					$default = Checkmydrive::_($default);
					$lang->setDebug($debug);
				}
				else
				{
	...
	protected function validateField(SimpleXMLElement $element, $group = null, $value = null, JRegistry $input = null)
	{
		...
		if ($required)
		{
			...
			if (($value === '') || ($value === null))
			{
				if ($element['label'])
				{
					$message = Checkmydrive::_($element['label']);
				}
				else
				{
					$message = Checkmydrive::_($element['name']);
				}

				$message = Checkmydrive::sprintf('JLIB_FORM_VALIDATE_FIELD_REQUIRED', $message);
		...
		if ($valid === false)
		{
			...
			if ($message)
			{
				$message = Checkmydrive::_($element['message']);
			...
			{
				$message = Checkmydrive::_($element['label']);
				$message = Checkmydrive::sprintf('JLIB_FORM_VALIDATE_FIELD_INVALID', $message);
