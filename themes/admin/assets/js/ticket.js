var TicketModel = (function($){
    ...
    return function(data){
        ...
        self.addComment = function(){
            ...
            if(!message) new PNotify({title: jSont._('CHECKMYDTIVE_ADD_COMMENT'), text: jSont._('CHECKMYDTIVE_MSG_MESSAGE_REQUIRED')});
            else{
                self.addCommentLoading(true);
                $.ajax({
                    type: 'post',
                    url: root + 'tickets/addComment',
                    data:{
                        id: self.ticket().id,
                        message: message,
                    ...
                    success: function(data){
                        ...
                        if(data.rs){
                            ...
                            new PNotify({title: jSont._('CHECKMYDTIVE_ADD_COMMENT'), text: data.msg});
                        }
                    }
                });
            }
        ...
        self.deleteComment = function(item){
            $.getJSON(root + 'tickets/deleteComment', {id: item.id}, function(data){
                if(data.rs){
